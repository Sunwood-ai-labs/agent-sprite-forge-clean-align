...
This is a local fork of `generate2dmap` that pairs with `generate2dsprite-clean-align` for cleanup-heavy prop and sprite handoff. This skill is for scenes and maps. Do not generate character, enemy, boss, projectile, NPC, player, or animation sprite assets as map deliverables. The map may include scene hooks such as player spawns, actor spawn marker metadata, patrol/encounter zones, arena entrances, gates, exits, and camera triggers, but actor artwork, projectiles, and animations belong in `$generate2dsprite-clean-align`.
...
4. For each visible runtime object, choose exactly one asset strategy:
   - generate a separate transparent asset with `$generate2dsprite-clean-align` or direct `image_gen`
...
Scene hooks are metadata only. Do not generate enemy, boss, NPC, player, projectile, or animation sprites inside `generate2dmap-clean-align`; call `$generate2dsprite-clean-align` separately when the game needs those assets.
