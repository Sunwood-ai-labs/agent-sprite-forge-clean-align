---
name: generate2dsprite-clean-align
description: "Fork of generate2dsprite focused on high-quality 2D sprite postprocessing: original-resolution 3x3 sprite sheets, improved magenta background cleanup, bbox and similarity-based frame alignment, transparent PNG sheets, animated WebP previews, and comparison HTML. Use when a generated sprite looks rough, has magenta fringes, frame jitter, unstable character placement, or needs side-by-side cleanup/alignment evaluation while preserving the original generate2dsprite workflow."
---

# Generate2dsprite Clean Align

This is a local fork of `generate2dsprite` for cleanup-heavy and alignment-heavy sprite work. Use the original `generate2dsprite` behavior for ordinary sprite generation, and prefer this fork when the user cares about original-resolution quality, background-removal artifacts, or animation stability.
...
- Preserve original generated resolution when the user cares about visual quality or says the sprite sheet looks rough. Do not downsample a raw `3x3` sheet to `64px` cells just for preview; either keep the raw cell size, or produce separate engine-scale exports and clearly label them.
- Prefer animated WebP over GIF when the user does not explicitly need GIF. GIF is palette-limited and often makes cleanup/fringe artifacts harder to judge.
- If users report magenta showing through, do not rely only on a hard `#FF00FF` threshold. Use color-distance cleanup plus transparent-edge fringe cleanup, and compare against the old hard-threshold result when evaluating.
- If users report animation jitter or "buggy" motion, compare at least direct split, bbox center/feet alignment, and alpha-mask similarity alignment. Keep a comparison HTML when judging which is best.
...
For high-quality `3x3` generated sheets where the raw image already has useful resolution, use the integrated comparison processor instead of immediately resizing to a small engine cell:

```bash
python scripts/process_3x3_alignment_variants.py \
  --raw-root <folder-with-asset-subfolders-containing-raw-sheet.png> \
  --output-root <comparison-output-folder> \
  --duration 240
```

This writes:

- `traditional/<asset>/`: direct 3x3 split with improved background cleanup
- `bbox/<asset>/`: bounding-box center X plus bottom/feet Y alignment
- `similarity/<asset>/`: alpha-mask similarity alignment against the middle frame
- `index.html`: side-by-side comparison of WebP animations and transparent sheets

Use this processor when:

- the user wants the original image resolution preserved
- the generated cells have position drift
- magenta/pink fringe remains after background removal
- the best alignment method is not obvious by inspection

For engine delivery, downscale or atlas-pack only after selecting the best cleaned/aligned method.
...
- did the processor preserve the intended resolution, or did it accidentally downsample the art
- did detached effects become noise
- does the sheet still read as one coherent animation
- does the subject center/feet line stay stable across frames
- does alpha-mask similarity alignment improve or over-stabilize the intended idle motion
- are magenta/pink fringes still visible on checkerboard backgrounds
...
- frame PNGs
- `animation-original.webp` or `animation.gif`, based on requested delivery format
- `prompt-used.txt`
- `pipeline-meta.json`
...
- `scripts/process_3x3_alignment_variants.py`: original-resolution 3x3 comparison processor with improved magenta cleanup, bbox alignment, similarity alignment, WebP export, and comparison HTML
