...
For `3x3` and `4x4` packs, create a layout-only guide first with `$generate2dsprite-clean-align`:

```bash
python skills/generate2dsprite-clean-align/scripts/make_layout_guide.py \
  --rows <ROWS> \
  --cols <COLS> \
  --cell-width 384 \
  --cell-height 384 \
